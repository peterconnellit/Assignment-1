//Immediately Invoked Function Expression (IIFE). A design pattern for restricting variables to local scope
(function iife() {
  ...
  /////////// REFACTORED APPEND FUNCTION FOR ARTICLES///////////
  ...
  //TEST CODE BELOW IS WORK IN PROGRESS, INTENDED TO REPLACE THE MULTIPLE ARTICLE FETCH FUNCTIONS

  /*Here is how I would refactor multiple fetch functions into one.
  The output is passed to console as I have not yet solved how to pass each array element to relevant article functions*/
  const fetchData = async () => {
    //Moved variables into local scope
    const apiKey = "93fa9c4c03804d35b9a4178db6eb4808";
    ...
    try {
      const response = await Promise.all(
        urlsArray.map((url) => fetch(url).then((res) => res.json()))
        /*The returned json data for each array element needs to be passed to the corresponding
        trending, gaming, tech and business article functions */
      );
      const json = response;
      const newsDataArr = Object.entries(json); //To Do
